...
public class CartGroupProductsTests : BaseTest, IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        Result<Guid, NotFoundError> response = await groupRepository.CreateGroup(Guid.NewGuid().ToString().Replace("-", ""), new HashSet<string>() { TestAuthHandler.UserEmail, TestAuthHandler.RandomEmail1 });
