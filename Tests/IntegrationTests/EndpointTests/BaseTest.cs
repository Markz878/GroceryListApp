...
public abstract class BaseTest : IDisposable
{
    ...
    protected async Task<Guid> CreateNewGroup(bool randomUser = false)
    {
        ...
        Result<Guid, NotFoundError> response = await groupRepository.CreateGroup(Guid.NewGuid().ToString().Replace("-", ""), new HashSet<string>() { randomUser ? TestAuthHandler.RandomEmail2 : TestAuthHandler.UserEmail, TestAuthHandler.RandomEmail1 });
