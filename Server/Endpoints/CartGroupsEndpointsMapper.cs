...
public static class CartGroupsEndpointsMapper
{
    ...
    public static async Task<Results<NoContent, ForbidHttpResult, NotFound>> DeleteCartGroup(Guid groupId, ClaimsPrincipal user, ICartGroupRepository cartGroupRepository)
    {
        NotFoundError? ex = await cartGroupRepository.DeleteCartGroup(groupId, user.GetUserEmail());
        return ex == null ? TypedResults.NoContent() : TypedResults.NotFound();
    }

    public static async Task<Results<NoContent, ForbidHttpResult, NotFound>> UpdateGroup(UpdateCartGroupNameRequest updatedGroup, ClaimsPrincipal user, ICartGroupRepository cartGroupRepository)
    {
        NotFoundError? ex = await cartGroupRepository.UpdateGroupName(updatedGroup.GroupId, updatedGroup.Name, user.GetUserEmail());
        return ex == null ? TypedResults.NoContent() : TypedResults.NotFound();
    }
}
